...
def find_best_match(patch, marged_img, is_compared, patch_x, patch_y, psi, search_area_size=100, 
		verbose=True, rotation=True, multi_processing=True):
    """
    Return the part of the picture with the lowest distance to the patch.
    """
    if multi_processing:
        result_x, result_y, result_r = findbestmatchmulti(patch, marged_img, is_compared, patch_x, 
...
def findbestmatchmulti(patch, img, is_compared, patch_x, patch_y, psi=9, search_area_size=200, 
		verbose=True, rotation=False):
    """
    Find the best match with parallelisation.
    """
    if rotation:
    	r_max = 4
    else:
    ...
    for i in range(num_processes):
    	p = Worker(args=(main_queue, result_queue, img, is_compared, gauss, patch, psi, verbose))
        jobs.append(p)
        p.start()
